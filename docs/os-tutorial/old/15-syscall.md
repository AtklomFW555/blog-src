如果你没用过 Linux，只做过 Windows 开发，那么没听说过系统调用（或者其简称syscall）是比较正常的，但没听说过API是不太合理的。在本篇文章中，你可以暂时认为这两者是一样的。??以目前 b 站科技区的典型视频来看，我感觉 winapi 的概念是个 Windows 用户应该都已经知道了吧……??
